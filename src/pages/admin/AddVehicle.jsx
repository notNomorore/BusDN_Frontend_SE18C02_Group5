...
const AddVehicle = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setSuccessMessage(data.message || 'Tạo xe thành công')
            setTimeout(() => {
                navigate('/admin/fleet-status')
            }, 800)
        } catch (error) {
            const responseErrors = error.response?.data?.errors
            setErrors(responseErrors?.length ? responseErrors : [error.response?.data?.message || 'Không thể tạo xe'])
        } finally {
            setSaving(false)
        }
    }
            ...
            <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                <div>
                    ...
                    <h1 className="mt-1 flex items-center gap-2 text-2xl font-bold text-gray-800">
                        <FaBus className="text-[#23a983]" /> Thêm xe mới
                    </h1>
                </div>
